...
extension TestSessionAdapter {
    // swiftlint:disable force_try
    func createData(fixtureFileName: String) -> Data {
        let resourceName = fixtureFileName.components(separatedBy: ".").first!
        let extensionName = fixtureFileName.components(separatedBy: ".").last!
        let fixtureFilePath = Bundle(for: type(of: self)).url(forResource: resourceName, withExtension: extensionName)!
        return try! Data(contentsOf: fixtureFilePath)
    }
}
